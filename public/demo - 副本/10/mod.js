(function (K) {
    var tabs = KClass.create({
        init: function (options) {
            ...
            this.fadetime = 50;
            this.to = 0; //slide的补间变量
            this.cur = 0; //fade的补间变量
            this.tevent = null;
            ...
            this.cons = opt.conCn ? K(this.conId).find(opt.conCn) : K(this.conId).find(opt.conTn); //切换页卡
            ...
            if (opt.effect == 'slide' || opt.effect == 'fxslide') {
                ...
                this.Tol = opt.vertical == 'top' ? 'top' : 'left'; /*控制垂直向左走,还是向右走..*/
                this.ml = this.cons.item(0).node[opt.vertical == "top" ? 'offsetHeight' : 'offsetWidth']; //计算应该要走的高宽度..
            }
        ...
        effects: {
            ...
            /*
            当滚动到第四个图片的时候，
            row2街上row1的尾部，继续向前滚动，
            row2滚到最后一个位置的时候，一次性转换成row1，
            然后循环..
            ...
            */
            ...
            flip: function (o, n) {
                ...
                var $clone = c.clone(true); //直接克隆当前显示图片节点
                var e = {
                ...
                flipObj = {
                    ...
                    top: c.parentX(), //距离父元素top
                    left: c.parentY(), //距离父元素left
                    target: o.opt.content || null, //翻转文字
                ...
                /*
                1,种，auto=1, 鼠标悬停，运动幕布执行完毕，auto清空,鼠标离开，继续auto
                2, auto=0 , 鼠标悬停，运动幕布执行完毕，幕布上不绑定任何事件, 鼠标离开..

                幕布正在执行的时候 o.scrolling=1, 
                幕布执行完毕o.scrolling=0;
                鼠标悬停，点击时候，如果认为幕布正在o.scrolling=1时候，不予执行，
                认为目标o.scrolling=0执行完毕时候，给予执行。

                处理翻滚幕布与图片的关系
                鼠标悬停，背景图片消失，只剩幕布在旋转，旋转完成之后，背景图片出现
                ...
                */
                ...
                // 上下翻滚控制
                var g = function () {
                    var a = (flipObj.height / 100) * 25; //(200/100)*25 改变幅度系数
                    var b = f();
                ...
                var scroll = function () {
                    ...
                    la();
                    dirOption = dirOptions[flipObj.direction](); //翻滚开始
            ...
            tabscroll: function (o, n, scrlen) {
                ...
                //点击的过程中如果存在运动线程，则继续执行现有的运动线程
                //如果不存在，则清楚运动线程..
                //重复点击，scrlen赋予多值，程式函数不知道到底走到哪一个scrlen,
                //所以会出现前后抖动的情况..晕..
                if (!o.end1) {
                    if (o.y < scrlen)
		...
		tabnext:function(o){ 
			...
			var n=o.current; 
            if (o.tabwr) {
                var scrlen = o.tabnum * o.tabitemlen*o.sn; //每次走多少距离.
				//还是得tab能一次走多少距离..
				if(o.sn<o.tn){ 
					if (o.opt.effect == 'def') {
        ...
        tabprev:function(o){ 
			...
			var n=o.current; 			
            if (o.tabwr) {
                var scrlen = o.tabitemlen*(o.levels/2); //每次走多少距离.
		...
		moveTo: function (n) {
            ...
            (n > this.levels - 1) && (n = 0);
            /*
				移动大图，带动小图移动scrollLeft的思路是正确的，下面是根据
				当前设置的shownum判断n到何处时候需要滚动...
				最关键的地方就是补间管理的时候如何把tabs滚动给停止住。
            */
            (n < 0) && (n = this.levels - 1);
            if (o.tabwr) {
				if(n==o.shownum){
	...
	var scroll = KClass.create({
                init: function (options){
					...
					o.scLeft=o.scrObj.node.scrollLeft;
					o.MoveLock=false;
					o.MoveTimeObj = null;
					o.AutoPlayObj = null;	
					o.scrEvent1=null;
					...
					this.PageWidth=o.vertical=='left'?o.scrWrap.child(0).node.offsetWidth*opt.scrNum:o.scrWrap.child(0).node.offsetHeight*opt.scrNum;
					this.finalMovePos=o.vertical=='left'?o.scrWrap.child(0).node.offsetWidth*(o.scrlen-o.showNum):o.scrWrap.child(0).node.offsetHeight*(o.scrlen-o.showNum);
				...
				disbtleft:function(o){
					/*
						disbtleft 要分清它是否有自动功能.
						如果有自动，那他在运动了一个窗格之后，要把自身激活
